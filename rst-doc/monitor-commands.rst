...
.. csv-table::
   ...
   ":ref:`interrupt <interrupt-command-label>`","display or change a PIO's IRQs configuration and status"
...
.. index::
   single: monitor command; interrupt
   single: interrupt

.. _interrupt-command-label:
...
^^^^^^^^^^^
  ...
  -s, --sm=NUMBER (mandatory: no)
            SM number, one of 0, 1, 2 or 3
  -i, --irq=NUMBER (mandatory: no)
            PIO IRQ number (0…7)
  -t, --txnfull (default: off)
            select TXNFULL interrupt flag of the selected state machine for modification
  -r, --rxnempty (default: off)
            select RXNEMPTY interrupt flag of the selected state machine for modification
  -d, --disable (default: off)
            override selected interrupt flag to be always cleared
  -e, --enable (default: off)
            revert "-d" option for the specifiedinterrupt flag
  -f, --force (default: off)
            override selected interrupt flag to be always set
  -u, --unforce (default: off)
            revert "-f" option for the specifiedinterrupt flag
  -0, --irq0 (default: off)
            select PIO IRQ0 as override target
  -1, --irq1 (default: off)
            select PIO IRQ1 as override target
  +v / -v, --value (mandatory: no)
            set value for the selected IRQ flag of those visible to all SMs
  -h, --help (default: off)
            display this help text and exit
...
Use option "-p"to select a PIO.
If none of the IRQ modification options is specified, the status
of the IRQ bits of the selected state machine is displayed.
For modification operation, additionally specify either option
"-s" for selecting a state machine within the selected PIO when
going to modify an SM specific IRQ flag.  Or specify option "-i"
for selecting one of those IRQ flags that are visible to all SMs.
Use option "-t" to apply modification on the TXNFULL group of
IRQ flags, or option "-r" to apply on the RXNEMPTY group of
IRQ flags; if none of those two options is specified, modification
will affect the SM group of IRQ flags.
For disabling the selected IRQ flag, use option "-d".
For enabling the selected IRQ flag, use option "-e".
For enforcing the selected IRQ flag to be set, use option "-f".
For undoing enforcement, use option "-u".
For selecting to which IRQ (IRQ0, IRQ1) option "-e", "-d",
"-f" or "-u" will apply, use option "-0" or "-1".
For setting or clearing one of the IRQs visible to all SMs, use
option "-v".
...
^^^^^^^^^^^
  ...
  --set-count=COUNT (mandatory: no)
            number of GPIO pins asserted by SET instruction (0…5)
  --set-base=NUMBER (mandatory: no)
            lowest-numbered GPIO pin affected by SET PINS / PINDIRS instruction (0…31)
  --out-count=COUNT (mandatory: no)
            number of GPIO pins asserted by OUT PINS / PINDIRS or MOV PINS instruction (0…32)
  --out-base=NUMBER (mandatory: no)
            lowest-numbered GPIO pin affected by OUT / MOV PINS / PINDIRS instruction (0…31)
  --in-base=NUMBER (mandatory: no)
            GPIO pin mapped to LSB for IN instruction (0…31)
  --jmp-pin=NUMBER (mandatory: no)
            GPIO pin to check when executing JMP PIN instruction (0…31)
  -h, --help (default: off)
            display this help text and exit
...
For setting pin count or pin base for SET / OUT / IN instructions or
the GPIO pin number to check when executing the JMP PIN instruction,
use the corresponding "--set-count", "--set-base",
"--out-count", "--out-base", "--in-base" or "--jmp-pin"
option.
This command does not support setting the side-set count or
